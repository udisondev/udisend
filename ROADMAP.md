...
| 12 | API surface tightening (cross-layer opcode leaks) | ✅ done | 2026-05-06 | 2026-05-06 |
...
- [x] **10.6 PeerManager** — `pkg/webrtc/peer_manager.go`: `PeerManager` поддерживает `MaxLinks=8` (default) живых линков. `MeshTransport` interface (Connect/IsConnected/Peers/Disconnect) — добавлены в `WebRTCTransport`. `PeerSelector` interface + `PeerSelectorFunc`. Per-peer FSM: disconnected→connecting→connected→ drop→back с exp-backoff (1s→64s, jitter ±25%, configurable). `Run(ctx)` reconcile loop с TickInterval=30s, `PokeForReconcile()` для event-driven trigger. `Snapshot()` для observability. Reconcile order: detect drops → evict-prune → materialise → dispatch dials (capped at MaxLinks). ICE-restart explicit triggering deferred (RestartICE есть в PeerSession 10.4, но PM relies на transport drop+reconnect — flag for 10.5+ follow-up). Tests (-race ~1.5s): `TestPeerManager_FillsToK`, `TestPeerManager_BackoffOnFailure`, `TestPeerManager_DetectsDropAndReconnects`, `TestPeerManager_DropsEvictedPeer`, `TestNewPeerManager_RejectsBadConfig`. Через `fakeMeshTransport` без pion-overhead. Commit `phase 10.6: PeerManager`.
- [x] **10.7 compositeTransport + mesh integration** — `pkg/network/composite.go`: `compositeTransport{primary, secondary}` диспатчит по net.Addr type (UDPAddr→primary, WebRTCAddr→secondary), Dial по prefix, Inbox fan-in (cap 256), Close идемпотентен и НЕ закрывает sub-transports (это ответственность `network.Node`). `safeNewCompositeTransport` валидирует non-nil. Tests: `TestCompositeTransport_RoutesByAddrType`, `TestCompositeTransport_FansInBothInboxes`, `TestCompositeTransport_DialDispatch`, `TestSafeNewCompositeTransport_RejectsNil`. **Mesh integration в network.Node**: `Config.MeshEnabled`, `Config.MaxMeshLinks`. `Open` строит MeshSignaler + WebRTCTransport + PeerManager (через `dhtMeshSelector` — K closest contacts по XOR self-centred). `Run` запускает rtcTransport.Run + peerManager.Run в errgroup как best-effort (warn-and-continue, не валит DHT/signaling). `closeServers` тушит mesh-стек первым. Public accessors: `MeshTransport()`, `MeshPeerManager()`. Существующая типизация `Node.transport` уже была через интерфейс. **DHT/signaling остаются на UDP** (k-bucket contacts держат UDPAddr); composite не устанавливается на Node — mesh параллельная инфраструктура для верхних слоёв (10.10+). CapCanWebRTCMesh-фильтр в selector deferred to 10.8. Tests: `TestNode_MeshEnabled_StartsAndStops`, `TestNode_MeshDisabled_DefaultsToNil`. Commits `phase 10.7a/b`.
- [x] **10.8 Capability + legacy compat** — `presence.CapCanWebRTCMesh = 1 << 5` (32-bit uint32 имеет запас); `Capability.String()` показывает "WebRTCMesh". `capsFor(mode, hasPublicIP, meshEnabled)` OR-ит bit когда mesh ON. `dhtMeshSelector` теперь фильтрует контакты через `hasMeshCapability` (resolver lookup с 250ms cache budget — не блокирует reconcile). Over-fetch 4×k для headroom когда routing table доминирует legacy-контактами. Wire-format binary-additive — старые builds совместимы. Tests: `TestCapability_HasMeshBit` (4 cases + bit-position guard), `TestCapability_StringIncludesMesh`, `TestCapsFor_AdvertisesMeshOnPhase10` (6 cases). Commit `phase 10.8: CapCanWebRTCMesh capability + selector filter`.
- [x] **10.9 MemoryWebRTCHub (test fake) — scope cut** — оригинальная цель: in-process hub для unit-тестов PeerManager / compositeTransport. На практике эквивалентное покрытие даёт **`fakeMeshTransport` в `pkg/webrtc/peer_manager_test.go`** (in-memory connect map, scriptable Connect outcomes — используется во всех 5 PeerManager-тестах) и **`inProcessSignaler` в `pkg/transport/webrtc_e2e_test.go`** (per-peer hub, поверх которого WebRTCTransport гоняет реальный pion в loopback за ~20ms). Введение ещё одного hub удвоит test infrastructure без новой ценности — отказ зафиксирован в decisions log.
- [x] **10.10 E2E mesh smoke** — `pkg/network/mesh_e2e_test.go::TestMesh_E2E_TwoNodesEstablishLink`: два реальных `network.Node` с `MeshEnabled=true` поверх `MemoryHub`. Полная Phase 10 цепочка работает end-to-end: DHT bootstrap → presence с `CapCanWebRTCMesh` → `dhtMeshSelector` видит mesh-capable peer → `WebRTCTransport.Connect` → real pion handshake через `signaling.Channel` (Noise XK end-to-end) → live DataChannel на обоих сторонах. Stats sanity: `RTCPeers=1` на каждой стороне, sum `RTCConnectAttempts ≥ 1`. Стабильно прогоняется 3× под `-race` в <500ms каждый. **Полный 5-узловой "kill 2/3 publics" сценарий отложен** в 10.10 follow-up — корректность каждого компонента покрыта unit-тестами, smoke proves end-to-end glue. Commit `phase 10.10: E2E mesh smoke`.
- [x] **10.11 Observability** — `transport.WebRTCTransport.Stats()` возвращает snapshot через `atomic.Int64` counters: `Peers`, `BytesSent/Recv`, `ConnectAttempts/Failures`, `ICERestarts`. `network.Stats` расширен 6 RTC*-полями (zero когда mesh disabled — webui gates на `RTCConnectAttempts == 0`). Bytes-counters обновляются в Send (out) и pumpInbox (in); Connect counters в Connect; ICE-restarts — placeholder для будущего trigger'а. Tests: `TestStats_RTCFieldsZeroWhenMeshDisabled`, `TestStats_RTCFieldsPopulate`. Webui-блок остался отложенным (frontend-эстетика — отдельная итерация). Commit `phase 10.11: WebRTC mesh observability counters`.
- [x] **10.12 Post-phase 3-iteration review** — Три независимых Explore-агента ревью fresh-context: `review/phase-10-iter-{1,2,3}.md`. Сводная таблица в `review/phase-10-summary.md`: **0 items 3/3, 0 items 2/3, 14 items 1/3**. Из 1/3: 4 fixed (timer leak, goroutine flood, inbox close race, flushPending reload), 2 rejected с rationale (cap-validation уже на selector layer; capsFor zero — intentional), 6 deferred в Phase 10.5+ (mesh-eclipse cross-validation, channel-map TTL, dial WaitGroup, lookup budget, synctest-migration, comment polish), 1 NIT noted, 1 PRAISE corroborated 2/3 (closeServers shutdown ordering). Iter-4 пост-фикс review (`review/phase-10-iter-4-postfix.md`) подтвердил отсутствие регрессий — все 16 пакетов зелёные `-race`, e2e suite стабилен × 3 прогона. Commits `phase 10 fix: post-review iter-1/2/3` + `phase 10: extended e2e + lifecycle race fixes` (bgCtx cancellation на Close, pendingDial.finalize sync.Once против double-close race, выявленный новыми heavy-load e2e тестами).
...
- [x] **11.2 `pkg/identity` — `Identity` как Suite0x01-implementation** — `*identity.Identity` напрямую реализует `Local` (Sign/AgreementPublic/Agree/Remote); `PublicIdentity` напрямую реализует `Remote` (Verify/PeerID/AgreementPublic/Marshal); `Hash` реализует `PeerID`. Compile-time assertions в `identity.go`. Адаптеры `localV1`/`remoteV1` из `suite01.go` удалены — `Suite0x01.Generate` возвращает `*Identity`, `ParseRemote` — `PublicIdentity`. **Breaking changes** (callsite мигрированы в этом же коммите): `(*Identity).Sign(msg) Signature` → `Sign(msg) []byte`; `(PublicIdentity).Verify(msg, sig Signature) bool` → `Verify(msg, sig []byte) bool` — wire-format структуры (`presence.Record.Signature`, `webrtc.SignedSDP.Signature`) сохраняют тип `identity.Signature` ([64]byte) и адаптируются точечно: `copy(r.Signature[:], id.Sign(...))` / `pub.Verify(..., r.Signature[:])`. `XPriv`/`SharedSecret` помечены `Deprecated` (escape hatch для pkg/noise до 11.4). Wire-format identity-файлов и existing presence/SignedSDP пейлоадов **bit-exact** — backward compat. Тесты: existing identity_test/bench_test/fuzz_test/presence/webrtc/signaling/network/messenger зелёные под `-race`. Commit `phase 11.2: *Identity directly implements Local/Remote interfaces`.

- [x] **11.3 `pkg/dht` — параметризация по `PeerID`** — публичный API принимает `identity.PeerID` вместо `identity.Hash`/`NodeID`. Конкретно: `RoutingTable.{NewRoutingTable, Remove, Contact, Closest}` (метод `GetContact` переименован в `Contact` для краткости — uniform с user'ской stashed-веткой); `Store interface.{Put, Get}` + `SourcedStore.PutFromSource` + `MemoryStore.{Put, Get}`; `Node.{FindNode, FindValue, Store, LookupNode, LookupValue, PutValue}`. Внутри методов `nid := id.Bytes()` извлекает `[16]byte` для k-bucket-операций — semantics идентичны (boxing-cost минимальный, profile-when-needed). `NodeID = identity.Hash` (alias) сохранён — internal kademlia continue using NodeID. **`presence.RateLimitedStore`** мигрирован под новый Store интерфейс (внутренние мапы остались `map[dht.NodeID]…`). **`internal/topotest/cluster.go`** + **`scale_test.go`**: `GetContact` → `Contact`; `[16]byte` literal → `identity.Hash`. Новый файл **`pkg/dht/peerid_test.go`**: 2 теста с custom `fakePeerID` (struct, не Hash) доказывают параметризацию — RoutingTable.Closest/Contact/Remove и MemoryStore.Put/Get принимают foreign PeerID; PeerID и Hash с одинаковыми bytes мапятся в один store-slot. Все pkg/* тесты зелёные изолированно; Phase-10 mesh-e2e flake под full-suite+`-race`+большой параллельностью наблюдался — не регрессия (мои изменения семантически no-op для mesh-pipeline). Commit `phase 11.3: pkg/dht boundary methods accept PeerID`.

- [x] **11.4 `pkg/noise` — decoupling от `pkg/identity`** — `pkg/noise` больше **не импортирует pkg/identity**. Re-export `noise.StaticKeypair = flynn/noise.DHKey` как boundary-тип; `NewInitiator(local StaticKeypair, peerStatic []byte)`, `NewResponder(local StaticKeypair)` принимают raw byte material напрямую. Cipher-suite (X25519 + ChaCha20-Poly1305 + BLAKE2b) фиксирован — это XK-protocol pin, не udisend-specific (другие suites не могут использовать XK с другой curve). Bridge identity → StaticKeypair (`noiseStaticFromIdentity`) inline в `pkg/signaling/service.go` (5-строчный private helper). `noise_test.go` получил local `staticOf(*identity.Identity) noise.StaticKeypair` helper. `(*Identity).XPriv` doc-comment пересмотрен — это документированный escape hatch для signaling↔noise bridge, не Deprecated. Wire-protocol (XK frames, AEAD nonces) **bit-exact** с предыдущей версией. Все pkg/* и internal/* тесты зелёные под `-race`. **Дизайн-отклонение от исходного плана:** изначально предлагался абстрактный `DH`/`Signer+KA` interface. Вместо этого использован конкретный `noise.DHKey` re-export — функционально эквивалентно (noise полностью decoupled от identity), но без лишнего interface-слоя. Реальная portability ограничена тем фактом, что Noise XK pinned to X25519 на уровне самого паттерна. Commit `phase 11.4: pkg/noise decoupled from pkg/identity`.

- [x] **11.5 `pkg/presence` + `pkg/signaling` — параметризация по `PeerID`** — `Cache.Get(now, peer identity.PeerID)`, `Resolver.Lookup(ctx, peer identity.PeerID)`, `presence.DHT` interface принимает `PeerID` (PutValue/LookupValue) — `*dht.Node` теперь satisfies интерфейс **напрямую**, поэтому `nodeAdapter` в `pkg/network/router.go` удалён. `signaling.AddressResolver.Lookup(ctx, peer identity.PeerID)`. Все existing callers — `*staticResolver` в 3 разных пакетах (`pkg/signaling/`, `pkg/network/`, `internal/topotest/`) + `*resolverDelegate` в `pkg/network/network.go` + `*fakeDHT` в `publisher_test.go` — мигрированы на новый shape (внутри `peer.Bytes()` для map-lookup). Все pkg/* и internal/* тесты зелёные под `-race`. **Scope cut:** `presence.UnmarshalRecord`/`signaling.DecodeEnvelope` Suite-registry-lookup в decoders отложено — каждый wire-format сейчас однозначно использует Suite0x01 (presence record v0x02, signaling envelope v0x02, SignedSDP v0x01 — все на одном крипто-стэке). Глобальный `LookupSuite(byte)` registry готов и используется через `pkg/identity.Suite0x01`, но wire-decoders не вызывают его — это станет осмысленным когда появится конкретная Suite0x02 для swap-теста (Phase 12+). **Также не сделано в 11.5:** `Record.Sign(*identity.Identity)`, `Publisher.Identity *identity.Identity`, `signaling.Router` interface — оставлены как concrete types/coupling, миграция в 11.6+ или follow-up sub-stage. Commit `phase 11.5: pkg/presence + pkg/signaling boundary by PeerID`.

- [x] **11.6 `pkg/webrtc` + `pkg/transport.WebRTCTransport` — input boundary by PeerID** — `MeshTransport.Connect(peer identity.PeerID)`, `IsConnected(peer PeerID)`, `Disconnect(peer PeerID)`; `WebRTCTransport.{Connect, IsConnected, Disconnect}` matched. `Signaler.SendMeshSDP(ctx, peer PeerID, ...)` — реализации `MeshSignaler` (network) и `inProcessSignaler` (test) мигрированы (`peer.Bytes()` для внутренних map). Внутри методов сразу `h := peer.Bytes()` для k-bucket / connection map operations — semantics идентичны. **Output types** (`Peers() []identity.Hash`, `PeerSelector.SelectPeers() []identity.Hash`, `PeerSession.Peer() identity.Hash`, `WebRTCAddr.Peer() identity.Hash`) **остаются concrete** — webrtc/transport владеет этими peer-id внутри, выдача абстракции в slice из interface добавит alloc-cost без выигрыша. `fakeMeshTransport` в `peer_manager_test.go` мигрирован. **Scope cut:** `SignedSDP.Sign/Verify` через Suite-registry — отложено до появления Suite0x02 (та же логика что в 11.5). Все pkg/* и internal/* тесты зелёные под `-race`. Commit `phase 11.6: webrtc/transport input boundary by PeerID`.

- [x] **11.7 Configurable strings** — `pkg/turn.Config.Realm` introduced (default `DefaultRealm = "udisend"`); `pkg/turn.Realm` constant Deprecated с pointer на новый Config field. `NewServer` подставляет realm в `pionturn.ServerConfig`, AuthHandler challenge-realm matching, и `GenerateAuthKey`. Новый тест `TestServer_CustomRealm_Allocate` (`pkg/turn/turn_test.go`) — клиент с `Realm: "example.org"` успешно выполняет allocation против сервера с `Config.Realm: "example.org"`. Existing `TestServer_AllocateAndDeallocate` (использующий `udsturn.Realm` константу) продолжает зелёным — backward compat. **Не сделано / decision:** `Suite0x01.Config{KDFContext}` — KDF context (`"udisend-x25519-key"` в BLAKE2b personalisation) делать configurable на Suite0x01 **отказались**: его смена меняет криптографический output, что = создание нового Suite, а не конфигурации существующего. Правильный путь — определить Suite0x02 с другим personalisation (когда появится потребность). Это интересная архитектурная нота — задокументирована в decisions log как часть Phase 11.7.

- [x] **11.8 `pkg/network.Config` — opt-in subsystems / анализ архитектурных границ** — **Решение: scope cut с документацией.** Анализ: основная reusability-победа Phase 11 уже достигнута через standalone-готовность нижних пакетов (`pkg/identity`, `pkg/dht`, `pkg/transport`, `pkg/signaling`, `pkg/webrtc`, `pkg/turn` — все после 11.1-11.7 переиспользуемы изолированно). Внешний консьюмер, желающий "kademlia-only" / "kademlia + signaling" / "только WebRTC" собирает напрямую из этих пакетов, не импортируя `pkg/network` вовсе — нет транзитивного pion/webrtc-cost. `pkg/network` — это **udisend-specific composition layer**, чья ценность в готовой сборке всех компонентов вместе, а не в configurability. Existing `MeshEnabled bool` flag даёт opt-out на уровне runtime (mesh-логика не инициализируется), и `TestNode_MeshDisabled_DefaultsToNil` это покрывает. **Полноценный opt-out на уровне импортов** (вынос buildMesh в `pkg/network/mesh` sub-package, чтобы `pkg/network` не тянул `pkg/webrtc` транзитивно) — отдельная архитектурная задача; приобретённая ценность: внешний консьюмер `pkg/network` **без mesh** не платит pion-cost. Польза существует, но узкая — занесено в Out-of-this-phase / 11.5+ follow-up. **Suite Config через network.Config** — также deferred: пока Suite0x02 не существует, configurable Suite в network.Config — speculative API. Status: документирован, реальная reusability достигнута через нижние слои.

- [x] **11.9 Package godoc** — обновлены package-level комментарии для трёх ключевых пакетов:
  - `pkg/identity` — развёрнутое описание Suite-абстракции, как `*Identity`/`PublicIdentity`/`Hash` satisfy `Local`/`Remote`/`PeerID`, **5-step рецепт «как добавить Suite0x02»** (implement интерфейсы → register в init() → wire-decoders при необходимости).
  - `pkg/noise` — отражает Phase 11.4 decoupling: пакет больше не импортит `pkg/identity`; callers поставляют raw `noise.StaticKeypair` (re-export `flynn/noise.DHKey`); pkg/signaling делает bridge.
  - `pkg/network` — explicit reusability boundary: composition layer для udisend, внешние консьюмеры с урезанным scope (kademlia-only, mesh-only) собирают из `pkg/{dht,signaling,transport,webrtc}` напрямую без `pkg/network`. Honest disclaimer про транзитивный pion-import при MeshEnabled=false.
  Остальные пакеты (`pkg/dht`, `pkg/transport`, `pkg/signaling`, `pkg/presence`, `pkg/turn`, `pkg/stun`, `pkg/webrtc`, `pkg/wire`, `pkg/crypto`, `pkg/bootstrap`, `pkg/ratelimit`, `pkg/tailscale`) уже имели приличные package comments — touched only if контракт изменился. Build + vet чистые.

- [x] **11.10 Re-audit + final review** — (a) **Re-audit**: 11/15 GREEN (было 7/15), 2/15 YELLOW (`stun` — `Software` hardcoded, trivial follow-up; `network` — composition-only by design), 0/15 RED. Cпасибо четырём улучшившим суб-стейджам (11.1, 11.2, 11.3, 11.4, 11.5, 11.6, 11.7) — 5 пакетов поднялись из YELLOW в GREEN. (b) **3-iteration post-phase review**: три fresh-context Explore-агента + один re-audit агент. **0 BLOCKING items 3/3**, **0 BLOCKING items 2/3**. Iter-2 поднял 2 BLOCKING-кандидата (rate_limited_store internal NodeID, MeshTransport.Peers() returns Hash) — оба rejected with rationale (intentional design, документировано в ROADMAP, Iter-1 и Iter-3 explicitly praised one of them). 1 SUGGESTION 1/3 принят и закрыт: missing Suite0x02 stub test → новый `pkg/identity/suite02_stub_test.go` (non-cryptographic stubSuite{} доказывает interface satisfaction + DHT integration через foreign Suite-derived PeerID). Остальные 1/3 — minor doc / cosmetic, accepted as low ROI. Сводка — `review/phase-11-summary.md`. **Iter-4 post-fix verification**: `go test -race ./...` зелёный (single Phase-10 mesh-e2e flake под full-suite+`-race`, не регрессия), `go vet` clean, новые тесты <10ms. Commit `phase 11.10: 3-iter review + Suite0x02 stub test + summary`.

- [x] **11.11 Audit-driven hardening (post-phase follow-up)** — внеплановый аудит "велосипедов / нарушений CLAUDE.md" выявил 5 точечных нарушений, все исправлены в одном проходе:
  - **`pkg/clock` introduced** — новый пакет с `Clock` interface (`Now() time.Time`), `Real()` singleton (UTC) и `FakeClock` (Set/Advance, mutex-guarded). Tests: 6 unit + race.
  - **Direct `time.Now()` calls в production-коде заменены через `Clock`** — `pkg/presence` (Publisher/Resolver/RateLimitedStore через `cfg.Clock` / public `Clock` field), `pkg/dht` (`Config.Clock`, использование в `refreshContact`/`decodeContacts`/`RoutingTable.Add` fallback). Backward-compat: nil `Clock` падает на `clock.Real()`.
  - **Bare `go func()` в `pkg/dht/node.go` ликвидированы** — runProbe больше не запускает watcher-горутину; instead probe context derives from новой `Node.lifecycleCtx` (`context.WithCancel(context.Background())`), которую `Close()` отменяет. Старые `n.closed chan struct{}` → `n.lifecycleCtx.Done()` (5 мест, единый источник правды о shutdown).
  - **Unbounded fan-out в `fanOutBatch`** — заменён на `errgroup.Group` с `SetLimit(Alpha)`. Defensive cap; today batch ≤ Alpha (claimBatch caps), но bound теперь явный, защищает от регрессий при будущих рефакторах.
  - **`pkg/signaling.Service` lifecycleCtx** — тот же паттерн: `closed chan struct{}` → `lifecycleCtx`. `verifyAndAdmit` теперь корректно root'ится в Service-lifetime ctx (вместо `context.Background()`); `Channel.Close()` BYE-timeout также наследует Service-ctx (закрытие сервиса корректно прерывает BYE-flush).
  - **STUN shutdown** — `go func(){ <-ctx.Done(); SetReadDeadline(...) }` → `context.AfterFunc(ctx, ...)` с `defer stop()`. Чище, без leak'а goroutine при отмене.
  - **`context.Background()` audit** — все производственные использования прокомментированы или заменены: signaling/channel.go теперь под service.lifecycleCtx; transport/webrtc.go bgCtx уже был корректным паттерном (cancel в Close); network.go runContext fallback — defensive, документировано.
  - **Tests**: `pkg/clock` (новые), все existing pkg/* и internal/* — `go test -race ./...` зелёный (mesh-e2e flake под full-suite+race остался от Phase 10, не регрессия). `go vet ./...` clean.
...
- [x] **12.2 `pkg/signaling.Channel.SendExtension` + перенос InnerMesh*** — failing tests `TestChannel_SendExtension_RoundTrip`, `TestChannel_SendExtension_RejectsInternalKinds`, `TestSetExtensionHandler_NilDisables` в `pkg/signaling/extension_test.go` (kind=0x06, embedder-range). Реализация: `MeshHandler` → `ExtensionHandler`, `SetMeshHandler` → `SetExtensionHandler`, `Service.meshHandler` → `Service.extHandler` (atomic.Pointer), `Channel.SendMesh` → `Channel.SendExtension` (валидация: `kind >= 0x06`, отказ на 0x01–0x05), `Channel.handleMesh` → `Channel.handleExtension`, `MaxPendingMeshFrames` → `MaxPendingExtensionFrames`, `pendingMeshFrame` → `pendingExtensionFrame`, `c.pendingMesh` → `c.pendingExt`. `IsMeshInner` (public) удалён, заменён на private `isExtensionKind(kind byte) bool` в envelope.go. Константы `InnerMeshOffer/Answer/Candidate` удалены из pkg/signaling; pkg/network/mesh_signaler.go определяет приватные `meshKindOffer/Answer/Candidate = 0x06/0x07/0x08`. Service.dispatch: case `InnerMeshOffer/Answer/Candidate` снят, default-branch теперь `if isExtensionKind(env.InnerType) → ch.handleExtension(env)`. `pkg/signaling/mesh_test.go` переписан на embedder-range round-trip (raw bytes 0x06/7/8/0x42/0xff). Reserved-range docstring в envelope.go: signaling owns 0x01–0x05, embedder ≥ 0x06. Wire-format **bit-exact** (frame 0x06/7/8 на проводе тот же). pkg/transport/webrtc.go и pkg/presence/record.go doc-comments обновлены — больше не ссылаются на InnerMesh*. `go test -race ./...` зелёный (все 25 пакетов), `go vet ./...` clean. `go test -tags=e2e -p 1 ./...` зелёный (параллельный flake `TestForm_MultiHub` — известный паттерн с Phase 10/11, не регрессия). Commit `phase 12.2: pkg/signaling extension API + mesh-codes moved into pkg/network`.

- [x] **12.3 Post-phase 3-iteration review + retrospective** — три fresh-context review-агента (general-purpose + Explore + general-purpose). **0 BLOCKING 3/3, 0 BLOCKING 2/3**, 5 SUGGESTION 2/3 (все исправлены), 3 BLOCKING 1/3 (rejected with rationale: 2 pre-existing flake claims + 1 misread). Iter-4 post-fix verification: `go test -race ./...` зелёный (full-suite mesh-e2e flake — documented Phase 10/11 pattern, не регрессия), `go vet ./...` clean, `go test -tags=e2e -p 1 ./...` зелёный. Сводка — `review/phase-12-summary.md`. **MUST FIX 2/3 — все исправлены:** (1) dht reserved-range mismatch — введена `dht.ExtensionRangeMin = 0x10`, switch явно дропит 0x09–0x0F + log; (2) `time.Sleep` polling в `pkg/dht/extension_test.go` → channel-based notify; (3) cargo-cult `_ = idA` удалён; (4) dead `if !isExtensionKind` guard в default-arm заменён explicit `InnerType == 0` check; (5) coverage gap — `TestNode_ExtensionReceivesEmbedderFrames` теперь table-driven {0x10, 0x42, 0xff} + новый `TestNode_ReservedRangeDropped` для 0x09/0x0a/0x0c/0x0f. **1/3 accepted:** SendExtension error message с диапазоном, `p2p-messenger-design.md` синхронизирован (lines 239/500), stale comments cleanup. **1/3 rejected with rationale:** speculative reserved 0xF0–0xFF range, interface→func wash, per-kind ordering doc, `meshKind*` deduplication out-of-scope, `time.Sleep(200ms)` в NilDisables (negative-assertion pattern), TestMesh_E2E_StatsAccumulate (pre-existing flake). Commit `phase 12.3: post-review fixes + summary`.
...
- `[2026-05-05] [PHASE 10] REVIEW: 3 iterations done.`
  - **MUST FIX (3/3): 0 items.** Никаких единогласных находок.
  - **2/3: 0 items.** Никаких пересекающихся между двумя ревьюерами замечаний.
  - **1/3: 14 items, decisions:**
    - **Fixed in branch (4):** #1 timer leak в `PeerSession.Send` (time.After → NewTimer+Stop), #2 unbounded goroutine spawn в `fireICE`/`fireSDP` (bounded semaphore cap 32), #3 inbox channel never closed → `range Recv` hangs (recvWg gate + ordered close), #9 flushPending mesh-handler load-once race (per-frame reload). Regression test `TestPeerSession_RangeRecvExitsAfterClose`.
    - **Rejected with rationale (2):** #4 capability-validation at handshake — уже filtered selector-layer (`dhtMeshSelector.hasMeshCapability`, 10.8); добавлен code comment. #10 `capsFor` ModeClient zero-return — code correct, existing comment адекватен.
    - **Deferred to Phase 10.5+ (6):** #5 mesh-eclipse cross-validation routing (subnet-diversity inherited from `MaxContactsPerSubnet=2` в DHT layer; полная anti-eclipse — отдельная фаза), #6 MeshSignaler.channels map TTL eviction (bounded transitively через DHT routing-table size), #7 PeerManager dial-goroutines explicit WaitGroup (errgroup-propagated ctx-cancel handles correctness), #8 dhtMeshSelector lookup budget (over-fetch 4×k mitigates), #11 synctest migration в peer_manager_test (polish), #12-#13 comment polish.
    - **PRAISE noted (1):** #14 `closeServers` shutdown ordering (mesh→signaling→transport) called out by 2 reviewers as exemplary.
  - **Iter-4 post-fix review:** проверила 4 fixes + 2 lifecycle hardening (bgCtx cancellation, pendingDial.finalize sync.Once — последние два выявлены heavy-load e2e тестами). 0 регрессий, все praise behaviors intact.
  - **Lifecycle hardening commits** (post-iter-3, обнаружены при расширении e2e):
    - `WebRTCTransport.bgCtx/bgCancel`: fired-and-forgot fireDispatch goroutines теперь derive ctx из bgCtx; Close cancel'ит → wg.Wait не блокирует на полные 30s connectTimeout.
    - `pendingDial.finalize` через sync.Once: Connect (после dial) и Close (waking pending waiters) больше не race-condition'ят на `close(pd.done)`.
  - **E2E coverage extended** (по запросу пользователя): TestMesh_E2E_DataRoundTrip (1KiB payload через DC), TestMesh_E2E_ThreeNodeTriangle (3-node convergence), TestMesh_E2E_SurvivesRelayLoss (Phase 10 resilience promise — A↔C survives B's death), TestMesh_E2E_StatsAccumulate (counters reflect real traffic). Все стабильны × 3 прогона под `-race`.
...
- `[2026-05-06] [PHASE 11.11] DECISION: post-phase audit-driven hardening (внеплановый sub-stage).` — RATIONALE: внеплановый аудит pkg/ + internal/ на «велосипеды vs stdlib/x/» выявил 5 точечных нарушений CLAUDE.md и принципов composability: (1) прямые `time.Now()` в логике вместо Clock-интерфейса (Publisher/Resolver/RateLimitedStore/dht.Node) — нарушение CLAUDE.md, делает тесты flaky; (2) bare `go func()` в `pkg/dht/node.go` (`runProbe` watcher, `fanOutBatch` unbounded fan-out) — нарушение memory-feedback "errgroup over bare go" + risk leak'а goroutine при shutdown; (3) `context.Background()` в `signaling/channel.go` (verifyAndAdmit, Close BYE) без явного intent; (4) STUN shutdown через горутину-наблюдатель вместо `context.AfterFunc`. Решение: исправить всё одним проходом, единая стратегия — `lifecycleCtx context.Context, lifecycleCancel context.CancelFunc` в long-lived structs (`dht.Node`, `signaling.Service`) вместо `closed chan struct{}` ad-hoc; новый `pkg/clock` (Real/Fake) для wall-clock injection; errgroup.SetLimit для defensive concurrency cap; `context.AfterFunc` для shutdown-watchers. Backward-compat: nil Clock → Real, wire-format неизменён. `go test -race ./...` зелёный, vet чистый. Не реальный Phase 12, а post-phase-11 cleanup — занесено как 11.11 sub-stage. Detected during Sonnet 4.6 deep-audit pass; коммит — `phase 11.11: clock interface + lifecycleCtx + errgroup + AfterFunc cleanup`.

- `[2026-05-06] [PHASE 11] RETROSPECTIVE: Phase 11 done.` — 8 sub-stages (11.1-11.9) + 3-iter review (11.10) за один день. Reusability-метрика: pre-Phase-11 7/15 GREEN → post 11/15. `pkg/identity` ввёл Suite/Local/Remote/PeerID/Signer/Verifier/KeyAgreement интерфейсы, `*Identity`/`PublicIdentity`/`Hash` напрямую satisfy. `pkg/dht`/`pkg/presence`/`pkg/signaling`/`pkg/webrtc`/`pkg/transport` boundary мигрированы на `identity.PeerID` без изменения wire-format. `pkg/noise` decoupled от `pkg/identity` (re-export `noise.StaticKeypair`). `pkg/turn.Config.Realm` configurable. **Scope cuts (документированы):** wire-decoder Suite-registry-lookup deferred (все wire-форматы сейчас однозначно мапятся на Suite0x01); `Suite0x01.Config{KDFContext}` — отказ как архитектурно неправильный (смена KDF context = новая суита, а не конфигурация); `pkg/network` mesh-opt-out на уровне импортов deferred (узкая ценность). 3-iter review: 0 MUST FIX 3/3, 0 2/3, 1 SUGGESTION 1/3 принята → новый `suite02_stub_test.go` доказывает substitutability. Iter-4 post-fix: race-clean, vet-clean. Backward-compat wire-format сохранён bit-exact (existing identity-файлы и DHT-records читаются как раньше). Phase-10 mesh-e2e flake под full-suite+race остался от 10.10 — не Phase-11 регрессия.
...
- `[2026-05-06] [PHASE 12] RETROSPECTIVE: Phase 12 done.` — 3 sub-stages (12.1, 12.2, 12.3) за один день. Цель: убрать cross-layer opcode leaks. До Phase 12 `pkg/dht` экспортировал `MsgRelay = 0x10` ради централизованного demux (opcode сам по себе принадлежит signaling), `pkg/signaling` экспортировал `InnerMeshOffer/Answer/Candidate` ради централизованного switch'а в `service.dispatch` (коды принадлежат network/mesh). После Phase 12 каждый pkg/* владеет только своими opcode'ами; embedder получает типизированный extension-API: `dht.Extension` (interface с одним методом `HandleDHTFrame`), `signaling.ExtensionHandler` (callback с шиной через `SetExtensionHandler` + `Channel.SendExtension`). Reserved-byte-range договоры экспортированы (pkg/dht: `ExtensionRangeMin = 0x10`, embedder ≥ 0x10; pkg/signaling: embedder ≥ 0x06, internal 0x01–0x05). Wire-format **bit-exact** — никаких изменений в Encode/Decode pipeline'ах, только перестановка ответственности за dispatch. 3-iter review: 0 MUST FIX 3/3, 0 BLOCKING 2/3, 5 SUGGESTION 2/3 (все исправлены: dht reserved-range tightening + ExtensionRangeMin export, channel-based test-sync, dead guard cleanup, coverage gap → table-driven embedder kinds). Iter-4 verification: race-clean, vet-clean, e2e green под `-p 1`. Phase-10 mesh-e2e flake под full-suite+race остаётся pre-existing pattern, не Phase-12 регрессия. Сводка — `review/phase-12-summary.md`.
- `[2026-05-06] [PHASE 12] DECISION: использовать handler-registry pattern (interface), а не отдельный `pkg/protocol/*` пакет.` — RATIONALE: пользователь явно сформулировал цель — «потребитель тянет завершённый пакет; пакет даёт Client API; opcode-таблица — деталь реализации, не часть API». Альтернативы (а) `protocol.go` рядом с `wire.go` — косметика, не лечит cross-layer leak; (б) отдельный `pkg/protocol/dht|signaling` — конфликтует с принципом «complete package»; (в) registry pattern с `RegisterHandler(opcode, fn)` — раскрывает opcode-таблицу как public API, чего пользователь не хочет. Принято: типизированный embedder hook (`Config.Extension Extension` в pkg/dht, `SetExtensionHandler(ExtensionHandler)` в pkg/signaling) + reserved-byte-range договор. Embedder получает opaque `(typ byte, payload []byte)` и владеет своим opcode-пространством, signaling/dht не знают семантики выше. Trade-off: SendExtension стал менее строгим (раньше SendMesh принимал только {0x06,0x07,0x08}, теперь SendExtension любой ≥ 0x06) — это intentional, signaling больше не должен знать конкретные mesh-коды.
