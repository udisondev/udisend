...
- [ ] 🚧 **Strict PING-back contact verification** — текущая защита: per-/24 bucket cap. Полная защита (контакт добавляется только после успешного PING-back challenge с nonce) требует хранения «candidates pool» и async-verify сценария. Per-/24 cap покрывает базовый Sybil; PING-back добавит резистентность к более продвинутым атакам.
