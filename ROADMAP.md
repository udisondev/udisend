...
## Phase 9 — Threat-model audit (open) — branch `feat/webui-remote-auth`

После публикации feat/webui-remote-auth прошла внутренняя проверка по комбинированной модели угроз: govulncheck (чисто), внешний обзор литературы (P2P/WebRTC/Argon2id/SQLite/Caddy), и три параллельных независимых ревью кода (HTTP/auth, P2P/crypto/storage, privacy/DoS/frontend). Большая часть HTTP-, auth- и frontend-уровневых проблем починена в этой ветке (см. decisions log 2026-05-05). Оставшиеся **protocol-level** находки требуют отдельной итерации:

### Out-of-this-PR / requires deeper protocol work

- [ ] 🚧 **DHT eclipse via unverified routing-table inserts** — `pkg/dht/node.go::refreshContact` доверяет attacker-supplied `SrcID` на любом входящем пакете. Нужен PING-back challenge (или signed-contact) перед добавлением в k-bucket; per-IP/24 cap внутри bucket; PoW-проверка `identity.PoWBits` на стороне DHT-приёма.
- [ ] 🚧 **DHT UDP amplification (~16x)** — FIND_NODE / FIND_VALUE отвечают ~800 байт на ~50-байтный запрос без хендшейка. Нужен BEP-42-style stateless cookie / source-validation первый раунд (PING — небольшой ответ — клиент эхом возвращает токен — затем FIND).
- [ ] 🚧 **Relay → iterative-lookup amplification** — `pkg/signaling/service.go::relay` + `pkg/network/router.go::NextHop` на cache-miss запускает полный `LookupNode` (alpha × disjoint goroutines). Нужен per-source rate-limit на relay-path, плюс drop-on-cache-miss policy для незнакомых recipient.
- [ ] 🚧 **Cold-cache `env.Sender` window** — `pkg/signaling/service.go::verifySenderIdentity` возвращает ok при resolver-error. Channel установлен и DATA-фрейм декодирован до того как Lookup завершится. На app-уровне закрыт SignedSDP, но pkg/signaling остаётся опасным API для будущих consumers.
- [ ] 🚧 **STORE без signed-only invariant на DHT-уровне** — pkg/dht не отказывает unsigned blob; всё держится на `presence.RateLimitedStore` обёртке. Stricter `Store` interface contract + sanity-check.
- [ ] 🚧 **Wire frame max-size > store max-value mismatch** — `wire.MaxFrameSize=1MiB` vs `MaxStoreValue=4KiB`; парсер аллоцирует до 1MiB перед бросанием. Bound at frame parser.
- [ ] 🚧 **TURN open-relay cap** — `pkg/turn` нет limit на allocation per source. Нужен per-source quota.
- [ ] 🚧 **Outbox TTL/cap** — `internal/storage`: outbox без TTL и size cap → forever-pending очередь от offline peer'а.
