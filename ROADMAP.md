...
- [ ] 🚧 **10.1 Scaffolding** — Phase 10 в ROADMAP, decisions log entries, `go get github.com/pion/webrtc/v4`, синхронизация `p2p-messenger-design.md` (раздел tech stack: pion/webrtc едет в network-binary; messenger по-прежнему через браузер). Стартер: `TestPhase10Scaffold/RoadmapHasPhase10`.
- [ ] **10.2 WebRTCAddr + transport skeleton** — `pkg/transport/webrtc.go`: `WebRTCAddr{peer identity.Hash}` (`Network()="rtc"`, `String()="rtc:<hex16>"`), `WebRTCTransport` struct с `ErrNotImplemented` stub'ами, sentinel errors (`ErrNoRoute`/`ErrAlreadyConnected`/`ErrBufferFull`), compile-time assertion `var _ transport.Transport`. Стартер: table-driven `TestWebRTCAddr_Roundtrip` + `FuzzWebRTCAddr_RoundTrip`.
- [ ] **10.3 Signaler + envelope extension** — `pkg/signaling/envelope.go`: `InnerMeshOffer=0x06`, `InnerMeshAnswer=0x07`, `InnerMeshICE=0x08`. `Signaler` interface в `pkg/transport/webrtc.go`. `pkg/network/mesh_signaler.go` — bridge поверх `signaling.Service`. Стартер: `TestEnvelope_DecodesMeshOffer`, `TestSignaler_RoundTripsOffer`.
- [ ] **10.4 PeerSession (pion обёртка)** — `pkg/webrtc/peer.go`: `PeerSession{pc, dc}` поверх `*pion.PeerConnection`, тонкие обёртки CreateOffer/Answer/SetRemoteDescription/AddICECandidate, `Send([]byte)` с bufferedAmount контролем, `Recv() <-chan []byte`, `Close()`, `RestartICE()`. Стартер: `TestPeerSession_OfferAnswerLoopback` (две PeerSession в одном процессе через канал-в-памяти, обмен 1KiB).
- [ ] **10.5 WebRTCTransport — полная реализация** — `Connect(ctx, peer)` с singleflight; `Send` strict-семантика; `Inbox()` fan-in из всех DC; goroutine на `Signaler.RecvMeshSDP()` роутит входящие. Стартер: `TestWebRTCTransport_ConnectAndSend`, `TestWebRTCTransport_SendWithoutConnectFails`, `TestWebRTCTransport_ConnectIdempotent`.
