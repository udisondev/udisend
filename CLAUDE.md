...
## Post-phase code review (обязательно)

После того как все acceptance criteria фазы зелёные и фаза готова к закрытию, **до перевода фазы в статус ✅ и до мержа** запускается процедура ревью.

### Процедура

1. **Три независимые итерации ревью** изменений фазы (используем skill `go-review` или `/ultrareview`, либо `Agent` с `subagent_type: code-reviewer`/`Explore` с явной просьбой ревью).
   - Каждая итерация — fresh-контекст, без подсказок что искать.
   - Каждая выдаёт список замечаний с severity: BLOCKING / SUGGESTION / NIT / PRAISE и file:line.
   - Замечания каждой итерации сохраняем (например, в `review/phase-N-iter-K.md` или прямо в коммит-сообщение фикса — главное не терять).

2. **Сводка по замечаниям** — после трёх итераций строится таблица:
   - **Подтверждено всеми 3 итерациями** → MUST FIX. Игнорировать нельзя ни при каких обстоятельствах.
   - **Подтверждено 2 итерациями** → разбираем содержательно: фиксим, либо явно отвергаем с обоснованием в decisions log.
   - **Только 1 итерацией** → принимаем решение, можно отложить, но решение фиксируем.

3. **Фикс блокеров** — все MUST FIX замечания исправляются перед закрытием фазы. Тесты на исправления — обязательны (TDD: failing test → fix → green).

4. **Повторный прогон** — после фиксов ещё одна (минимум) итерация ревью на изменённые места, чтобы убедиться: новые баги не внесены, MUST FIX действительно закрыты.

5. **Только после этого** — фаза переводится в ✅, в decisions log добавляется запись формата:
   ```
   [DATE] [PHASE N] REVIEW: 3 iterations done.
   - MUST FIX (3/3): N items, все исправлены коммитами <hash1, hash2, ...>.
   - 2/3: M items, решения: <fixed | rejected with rationale>.
   - 1/3: K items, решения: <ditto>.
   ```

### Жёсткие правила

- **Ни одно замечание, подтверждённое всеми 3 итерациями, не может быть проигнорировано.** Если кажется, что замечание неприменимо — значит ревьюеры что-то поняли не так из-за кода → код надо переписать так, чтобы намерение было однозначно.
- Не считаем итерацию состоявшейся, если она была подсказана предыдущей (например, я отправил агенту список из прошлой итерации с просьбой "перепроверь"). Каждая итерация смотрит на код с нуля.
- Ревью обязательно для **каждой** фазы roadmap, кроме Phase 0 (bootstrap — там нет логики).
