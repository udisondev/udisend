...
func ExtractPublicAuth(identifier string) (*ecdsa.PublicKey, error) {
	if len(identifier) < 26 {
		return nil, errors.New("identifier too short")
	}

	base64Part := identifier[26:]

	pemBytes, err := base64.StdEncoding.DecodeString(base64Part)
	if err != nil {
